...
class GAT(torch.nn.Module):
    def __init__(self, num_node_features, hidden_dim, num_classes):
        ...
        self.conv3 = GATConv(hidden_dim * 8, num_classes, heads=1, dropout=0.6)
...
def dsatur_coloring_with_gat_order(edge_index, node_embeddings):
    import heapq
    ...
    scores = node_embeddings.norm(dim=1).tolist()
    node_order = sorted(range(num_nodes), key=lambda x: scores[x], reverse=True)

    colors = [-1] * num_nodes
    saturation = [0] * num_nodes
    degrees = [G.degree[i] for i in range(num_nodes)]
    neighbor_colors = [set() for _ in range(num_nodes)]

    for node in node_order:
        used_colors = set(colors[neighbor] for neighbor in G[node] if colors[neighbor] != -1)
        ...
        for neighbor in G[node]:
            if colors[node] not in neighbor_colors[neighbor]:
                neighbor_colors[neighbor].add(colors[node])
...
learning_rate = 0.005
num_epochs = 5300
output_dir = 'table1_results_32hd_3lay_dsatur'
...
for instance in instances:
    ...
    model.eval()
    out = model(data.x, data.edge_index)
    # GAT Embedding to DSATUR Coloration
    with torch.no_grad():
        node_embeddings = out.detach().cpu()
        edge_list = edge_index.cpu()
        dsatur_colors = dsatur_coloring_with_gat_order(edge_list, node_embeddings)

    predicted_colors = torch.tensor(dsatur_colors)
